...
#This version of the script divides the data into chromosomes. It also requires that an individual have data in atleast 10 sites within a window, or it doesn't print that sample.  
use strict;
use warnings;
...
#        $loci_count++;
	if ($currentchr ne $a[0]){
	        open(my $g_file, '>', "${out}.chr.${currentchr}.hiest.G");
        	open(my $p_file, '>', "${out}.chr.${currentchr}.hiest.P");
		print $g_file "sample";

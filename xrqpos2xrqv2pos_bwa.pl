...
my $xrqv2_ref = $ENV{'XRQv2_REF'} // "/moonriseNFS/XRQ/HanXRQr2/HanXRQv2";
my $xrq_ref   = $ENV{'XRQ_REF'}   // "/moonriseNFS/XRQ/HanXRQr1.0-20151230.fa";
...
while(<SAM>){
  ...
  my ($start_soft_clip) = $cigar =~ /^(\d+)[SH]/;
  my ($end_soft_clip) = $cigar =~ /(\d+)[SH]$/;
  #my @softclip = split(/S/, $code);
  #my $clip_value = 0;
  #if ($softclip[0] =~  /^[0-9]+$/ ){
  #  $clip_value = $softclip[0];
  #}
  unless($end_soft_clip){
   $end_soft_clip = 0;
  }
  unless($start_soft_clip){
   $start_soft_clip = 0;
  }
  my $target_snp;
  if ($flipped eq "forward"){
    $target_snp = $target_pos + $bases_surrounding - 2 - $start_soft_clip; #Modified to fix targetting problem
  }else{
    $target_snp =  $target_pos + $bases_surrounding + 2 - $end_soft_clip; #For reverse
  }
  $loc_hash{$orig_chr.$orig_snp}{"chr"} = $target_chr;
  $loc_hash{$orig_chr.$orig_snp}{"pos"} = $target_snp;
  $loc_hash{$orig_chr.$orig_snp}{"rc"} = $flipped;
  $loc_hash{$orig_chr.$orig_snp}{"clipped"} = "good";
  if (($start_soft_clip > 0) or ($end_soft_clip > 0)) {
     $loc_hash{$orig_chr.$orig_snp}{"clipped"} = "uncertain";
  }
  if ($cigar eq "201M"){
     $loc_hash{$orig_chr.$orig_snp}{"clipped"} = "perfect";
  }
...
while(<INPUT>){
  ...
  if ($loc_hash{$chr.$pos}{"chr"}){
    until (!$used_sites{$loc_hash{$chr.$pos}{'chr'}.$loc_hash{$chr.$pos}{'pos'}}){ #If the specific location was already used, shift the position by one.
     ...
     foreach my $i (2..4){
       if ($loc_hash{$chr.$pos}{'rc'} eq "reverse"){
         my $allele = $a[$i];
         $allele =~ tr/ATCGatcg/TAGCtagc/;
         print $unsorted_vcf "\t$allele";
       }else{
     ...
     print $unsorted_vcf "XRQ=$chr.$pos.$loc_hash{$chr.$pos}{'rc'}.$loc_hash{$chr.$pos}{'clipped'}";
     foreach my $i (8..$#a){
       print $unsorted_vcf "\t$a[$i]";
     }

...
while(<INFO>){
  ...
  if ($a[1]){
    ...
    my $file = "$output.$a[0].input";
    open $fh{$a[0]},'>', $file or die "Can't open the output file: $!";
  }
}
  ...
  if ($line=~m/^#/){
    ...
    foreach my $i (9..$#a){
      if ($group{$name{$i}}){
        ...
        print { $fh{$name{$i}} }"$chr\t$pos\t$ref_genotypes{1}{0}\t$ref_genotypes{1}{1}\t$ref_genotypes{2}{0}\t$ref_genotypes{2}{1}\t$morgan_dist\t$reads[0]\t$reads[1]\n";
